...
print("  安装依赖(可能需要几分钟)...")
out, err = run(f'{REMOTE_DIR}/venv/bin/pip install --upgrade pip pymatgen scikit-learn joblib numpy 2>&1 | tail -3', timeout=600)
print(f"  {out}")

run(f'chmod +x {REMOTE_DIR}/setup_server.sh')

print("\n验证安装...")
out, err = run(f'cd {REMOTE_DIR} && venv/bin/python -c "import stacking_analyzer; print(f\'pymatgen={stacking_analyzer.HAS_PYMATGEN}, sklearn={stacking_analyzer.HAS_SKLEARN}\')" 2>&1')
print(f"  {out}")
