...
campus_run(
    f'timeout 60 bash -c "echo \\"\\\\" | nc {CLOUD_HOST} 22" 2>&1 || echo "NC_TIMEOUT"', timeout=70
)

print("\n  Checking if SSH is on alternate ports...")
campus_run(
    f'timeout 3 bash -c "echo > /dev/tcp/{CLOUD_HOST}/2222" 2>&1 && echo "Port 2222 OPEN" || echo "Port 2222 CLOSED"',
    timeout=5,
)
campus_run(
    f'timeout 3 bash -c "echo > /dev/tcp/{CLOUD_HOST}/8022" 2>&1 && echo "Port 8022 OPEN" || echo "Port 8022 CLOSED"',
    timeout=5,
)
