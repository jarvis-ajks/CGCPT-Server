...
# 2. API
code, r = run("curl -s http://localhost/CGCPT/api/stats | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d.get(\"total_materials\",0))'")
...
try:
    h = json.loads(r)
    tests.append(("Health check", h.get('status') == 'ok', f"uptime={h.get('uptime_seconds')}s, mem={h.get('memory',{}).get('percent',0)}%"))
except:
...
code, r = run("curl -sI http://localhost/CGCPT/assets/vendor-react-076Dd0Bx.js | grep -i cache-control")
tests.append(("Asset cache", 'immutable' in r or '365' in r, r.strip()))

# 6. Brotli
code, r = run("curl -sI -H 'Accept-Encoding: br,gzip' http://localhost/CGCPT/assets/vendor-react-076Dd0Bx.js | grep -i content-encoding")
tests.append(("Brotli/gzip", 'br' in r or 'gzip' in r, r.strip()))

# 7. Security headers
code, r = run("curl -sI http://localhost/CGCPT/ | grep -i 'x-frame\\|x-content'")
tests.append(("Security headers", 'X-Frame' in r or 'X-Content' in r, r.strip() if r.strip() else 'none'))
...
print("\n=== Performance Comparison ===")
code, r = run("curl -s -o /dev/null -w '%{time_total}s %{size_download}B' http://localhost/CGCPT/assets/vendor-react-076Dd0Bx.js")
print(f"React vendor JS: {r}")

code, r = run("curl -s -o /dev/null -w '%{time_total}s %{size_download}B' -H 'Accept-Encoding: br,gzip' http://localhost/CGCPT/assets/vendor-react-076Dd0Bx.js")
print(f"React vendor JS (compressed): {r}")

code, r = run("curl -s -o /dev/null -w '%{time_total}s %{size_download}B' -H 'Accept-Encoding: br,gzip' http://localhost/CGCPT/assets/vendor-three-Kl_qsPTx.js")
