...
code, r = run(
    "curl -s -o /dev/null -w '%{http_code}' http://localhost/CGCPT/assets/vendor-react-076Dd0Bx.js"
)
ok = r == "200"
...
# 3. Three.js chunk
code, r = run(
    "curl -s -o /dev/null -w '%{http_code}' http://localhost/CGCPT/assets/vendor-three-Kl_qsPTx.js"
)
ok = r == "200"
...
# 4. CSS
code, r = run(
    "curl -s -o /dev/null -w '%{http_code}' http://localhost/CGCPT/assets/index-DqPzHtd4.css"
)
ok = r == "200"
...
try:
    ...
    tests.append(
        (
            "API /stats",
            ok,
            f"materials={d.get('total_materials')}, topologies={d.get('unique_topologies')}",
        )
    )
except:
...
# 6. API prototypes
code, r = run(
    "curl -s http://localhost/CGCPT/api/prototypes | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d.get(\"total\",0))'"
)
...
for tf in test_files:
    ...
    if len(parts) >= 2:
        ...
        total += 1
        print(
            f"  {tf}: pred={pred}, expected={expected}, conf={conf}, {'OK' if is_correct else 'WRONG'}"
        )
