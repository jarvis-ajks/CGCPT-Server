...
campus_sftp.put(os.path.join(LOCAL_BASE, 'stacking_analyzer.py'), '/archive/jarvisajks/cgcpt-relay/stacking_analyzer.py')
print("  OK")

print("\n[4/7] 上传api_server.py...")
campus_sftp.put(os.path.join(LOCAL_BASE, 'api_server.py'), '/archive/jarvisajks/cgcpt-relay/api_server.py')
print("  OK")

print("\n[5/7] 上传模型...")
campus_sftp.put(os.path.join(LOCAL_BASE, 'models', 'gb_97393.pkl'), '/archive/jarvisajks/cgcpt-relay/gb_97393.pkl')
print("  OK")
...
for tf in os.listdir(test_dir):
    if tf.endswith('.cif'):
        campus_sftp.put(os.path.join(test_dir, tf), f'/archive/jarvisajks/cgcpt-relay/test_cifs/{tf}')
        print(f"  {tf}")

# Ensure paramiko is installed on campus server
print("\n[7/7] 在校园网服务器上执行中转部署...")
out, _ = campus_run('/archive/jarvisajks/cgcpt-stacking/venv/bin/python -c "import paramiko; print(paramiko.__version__)" 2>&1')
if 'module' in out.lower() or not out:
    print("  安装paramiko到校园网服务器...")
    campus_run('/archive/jarvisajks/cgcpt-stacking/venv/bin/pip install -i https://mirrors.aliyun.com/pypi/simple/ --trusted-host mirrors.aliyun.com paramiko 2>&1 | tail -3', timeout=120)

print("  执行中转部署脚本...")
out, err = campus_run('/archive/jarvisajks/cgcpt-stacking/venv/bin/python /archive/jarvisajks/cgcpt-relay/relay_deploy.py 2>&1', timeout=600)
