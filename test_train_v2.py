...
out, err = run('curl -s -X POST http://127.0.0.1:5001/api/stacking/train -H "Content-Type: application/json" -d \'{"test_ratio": 0.2, "model_type": "auto", "n_iterations": 2, "cv_folds": 3}\'')
data = json.loads(out)
...
if data.get('success'):
    ...
    for t, info in sorted(mc.items(), key=lambda x: x[1]['best_acc'], reverse=True):
        print(f'  {info["name"]}: best={info["best_acc"]:.4f} avg={info["avg_acc"]:.4f} ({info["count"]} tests)')
