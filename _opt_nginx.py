...
code, r = run(
    "apt-get install -y -qq libnginx-mod-http-brotli-filter libnginx-mod-http-brotli-static 2>&1 | tail -5"
)
print(r)

# Check if brotli modules are available
code, r = run(
    "ls /etc/nginx/modules-available/ 2>/dev/null | grep brotli; ls /usr/lib/nginx/modules/ 2>/dev/null | grep brotli"
)
...
"""
    if "brotli on" not in new_conf:
        new_conf = new_conf.replace(
            "    gzip_min_length", brotli_settings + "\n    gzip_min_length"
        )
...
if "/CGCPT/assets/" not in current_config:
    ...
    new_config = current_config.replace(
        "location /CGCPT/api/", cgcpt_assets_config + "\n        location /CGCPT/api/"
    )
    code, r = run(
        f"""cat > /tmp/ai-website-new << 'NGINXEOF'
{new_config}
NGINXEOF
"""
    )
    code, r = run(
        "cp /etc/nginx/sites-available/ai-website /etc/nginx/sites-available/ai-website.bak"
    )
...
if "successful" in r:
    ...
    # Restore backup
    code, r = run(
        "cp /etc/nginx/sites-available/ai-website.bak /etc/nginx/sites-available/ai-website"
    )
