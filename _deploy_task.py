...
out, err = run(
    f"mkdir -p {REMOTE}/web/dist && cd {REMOTE}/web/dist && tar xzf {REMOTE}/frontend.tar.gz && rm {REMOTE}/frontend.tar.gz"
)
print("OK")
...
print("\n[5/5] 验证新API...")
out, err = run(
    "curl -s http://localhost:5001/CGCPT/api/stacking/improvement_history | python3 -m json.tool"
)
print("  --- API响应 ---")
print(out[:3000])
if err:
