...
if os.path.isdir(dist_dir):
    ...
    run(
        f"cd {REMOTE_BASE}/root/CGCPT && tar xzf {REMOTE_BASE}/frontend_new.tar.gz && rm {REMOTE_BASE}/frontend_new.tar.gz"
    )
    os.remove(tmp_tar)
...
run(
    'curl -s -o /dev/null -w "JS: %{http_code} %{size_download}" http://127.0.0.1/CGCPT/assets/index-_UHI_a-C.js'
)
run(
