...
print(f"  pip: {out}")
out, err = run(f'{remote_dir}/venv/bin/pip install pymatgen scikit-learn joblib numpy 2>&1 | tail -3', timeout=600)
print(f"  依赖: {out}")

print("\n验证...")
out, err = run(f'cd {remote_dir} && venv/bin/python -c "import stacking_analyzer; print(\'pymatgen=\', stacking_analyzer.HAS_PYMATGEN, \'sklearn=\', stacking_analyzer.HAS_SKLEARN)" 2>&1')
print(f"  {out}")
if err:
    print(f"  err: {err[:300]}")
