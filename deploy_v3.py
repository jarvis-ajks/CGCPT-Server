...
run(
    f"cd {REMOTE_BASE}/root/CGCPT && tar xzf {REMOTE_BASE}/frontend_v3.tar.gz && rm {REMOTE_BASE}/frontend_v3.tar.gz"
)
os.remove(tmp_tar)
print("Frontend deployed!")

sftp.put(
    os.path.join(LOCAL_BASE, "stacking_analyzer.py"), f"{REMOTE_BASE}/backend/stacking_analyzer.py"
)
...
tests = [
    ...
    (
        "API-Stats",
        f'curl -s http://127.0.0.1/CGCPT/api/stats | python3 -c "import sys,json;d=json.load(sys.stdin);print(f\\"Materials: {{d[\\\\"total_materials\\\\"]}}, SpaceGroups: {{d.get(\\\\"unique_space_groups\\\\",0)}}\\")"',
    ),
]
