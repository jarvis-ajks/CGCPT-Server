...
endpoints = [
    ...
    (
        "GET /api/prototypes",
        "curl -s -o /dev/null -w '%{time_total}s' http://localhost:5001/api/prototypes",
    ),
    (
        "GET /api/materials",
        "curl -s -o /dev/null -w '%{time_total}s' 'http://localhost:5001/api/materials?per_page=20'",
    ),
    (
        "GET /api/elements",
        "curl -s -o /dev/null -w '%{time_total}s' http://localhost:5001/api/elements",
    ),
    (
        "GET /api/lattice-types",
        "curl -s -o /dev/null -w '%{time_total}s' http://localhost:5001/api/lattice-types",
    ),
    (
        "GET /api/stacking/models",
        "curl -s -o /dev/null -w '%{time_total}s' http://localhost:5001/api/stacking/models",
    ),
]
...
print("\n=== 5. nginx Config ===")
code, r = run(
    "cat /etc/nginx/sites-available/ai-website | grep -E 'gzip|cache|buffer|proxy_read|client_max' | head -15"
)
print(r)
...
code, r = run(
    "ps aux | grep gunicorn | grep -v grep | awk '{printf \"PID=%s RSS=%sMB CPU=%s%% CMD=%s\\n\", $2, int($6/1024), $3, $11}'"
)
print(r)
