...
conn.on('ready', function() {
  ...
  var steps = [
    'echo "Step 1: Remove conflicting cgcpt.conf"',
    'rm -f /etc/nginx/conf.d/cgcpt.conf',
    'echo "Removed cgcpt.conf"',

    'echo "Step 2: Create symlink for frontend"',
    'rm -rf /opt/CGCPT/root 2>/dev/null',
    'ln -sf /opt/CGCPT/web/dist /opt/CGCPT/root',
    'ls -la /opt/CGCPT/root/index.html 2>/dev/null && echo "Frontend symlink OK" || echo "Symlink failed"',

    'echo "Step 3: Check API directly"',
    'curl -s http://127.0.0.1:5001/health 2>&1',
    'curl -s http://127.0.0.1:5001/api/health 2>&1',

    'echo "Step 4: Update ai-website nginx config - fix CGCPT section"',
  ];

  var idx = 0;
  function runNext() {
    if (idx >= steps.length) {
      updateAiWebsiteConfig();
      return;
    }
  ...
  function updateAiWebsiteConfig() {
    var newAiWebsite = [
      'server {',
    ...
    conn.exec(writeCmd, function(e, st) {
      ...
      st.stderr.on('data', function(d) { o += d.toString(); });
      st.on('close', function() {
        console.log('Written ai-website config');
        ...
        conn.exec('nginx -t 2>&1', function(e, st) {
          ...
          st.on('close', function() {
            ...
            if (o.indexOf('successful') >= 0 || o.indexOf('ok') >= 0) {
              ...
              console.log('Nginx test FAILED - reverting');
              conn.end();
            }
          });
        });
  ...
  function verify() {
    ...
    var mobile = 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1';
