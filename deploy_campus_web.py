...
run(
    f"rm -rf {REMOTE}/frontend/* && cd {REMOTE}/frontend && tar xzf {REMOTE}/frontend.tar.gz && rm {REMOTE}/frontend.tar.gz"
)
os.remove(tmp_tar)
print("  OK")
...
print("\n[4/8] 上传模型...")
sftp.put(
    os.path.join(LOCAL_BASE, "models", "gb_97393.pkl"), f"{REMOTE}/backend/models/gb_97393.pkl"
)
print("  OK")
...
run(
    f"{REMOTE}/venv/bin/pip install -i https://mirrors.aliyun.com/pypi/simple/ --trusted-host mirrors.aliyun.com flask flask-cors gunicorn pymatgen scikit-learn joblib numpy 2>&1 | tail -3",
    timeout=600,
)
print("  依赖安装完成")
...
for tf in test_files:
    ...
    out, _ = run(
        f"""cd {REMOTE}/backend && {REMOTE}/venv/bin/python -c "
import sys
sys.path.insert(0, '.')
