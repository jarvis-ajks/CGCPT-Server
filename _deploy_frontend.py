...
run(f'cd {REMOTE_BASE}/root/CGCPT && tar xzf {REMOTE_BASE}/fe_deploy.tar.gz && rm {REMOTE_BASE}/fe_deploy.tar.gz')
...
out, _ = run(f'curl -sI -H "Accept-Encoding: br" http://localhost/CGCPT/assets/vendor-three-*.js 2>&1 | grep -i "content-encoding"')
