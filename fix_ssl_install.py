...
for i, mirror_args in enumerate(mirrors):
    ...
    out2, _ = run(
        f"{VENV}/bin/python -c \"import numpy; print('numpy OK:', numpy.__version__)\" 2>&1"
    )
    if "OK" in out2:
...
print("\n最终验证...")
out, _ = run(
    f"cd /archive/jarvisajks/cgcpt-stacking && {VENV}/bin/python -c \"import stacking_analyzer; print('pymatgen=', stacking_analyzer.HAS_PYMATGEN, 'sklearn=', stacking_analyzer.HAS_SKLEARN)\" 2>&1"
)
print(f"  {out}")
