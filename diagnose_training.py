        ...
        if no_overlap:
            ...
            for t in topos:
                if t in unique_ranges:
                    print(
                        f"       {t[:40]:>40s}: [{unique_ranges[t][0]:.4f}, {unique_ranges[t][1]:.4f}]"
                    )
...
for fk in ["n_unique_elements", "x_to_o_ratio", "o_count", "non_o_count"]:
    ...
    for t in topos:
        vals = [
            s["features"][fk]
            for s in samples
            if s["topology"] == t and s["features"].get(fk) is not None
        ]
        if vals:
            unique_vals = sorted(set(float(v) for v in vals))
            print(
                f"    {t[:40]:>40s}: unique={unique_vals[:10]}{'...' if len(unique_vals)>10 else ''} (n_unique={len(unique_vals)})"
            )
