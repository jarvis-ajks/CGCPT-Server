...
for tf_path in test_files:
    ...
    out, err = run(f'''curl -s -X POST http://127.0.0.1:5001/api/stacking/predict \
      -H "Content-Type: application/json" \
      -d "$(python3 -c "import json; f=open('{tf_path}'); d=f.read(); f.close(); print(json.dumps({{'model_id':'gb_97393','cif_text':d}}))")" 2>&1''', timeout=60)
    ...
    try:
        ...
        print(f"  {match} {fname[:50]}: predicted={pred}, confidence={conf:.2%}, expected={expected}")
    except:
...
try:
    ...
    for m in models:
        print(f"    {m['model_id']}: acc={m.get('test_accuracy',0)}, type={m.get('model_type','?')}, samples={m.get('n_samples',0)}")
except:
    print(f"  {out[:200]}")
