...
run(
    f"cd {REMOTE_BASE}/root/CGCPT && tar xzf {REMOTE_BASE}/frontend_mobile.tar.gz && rm {REMOTE_BASE}/frontend_mobile.tar.gz"
)
os.remove(tmp_tar)
...
run(
    'curl -s -o /dev/null -w "JS: %{http_code} %{size_download}" http://127.0.0.1/CGCPT/assets/index-D1vw7YrE.js'
)
run(
    'curl -s -o /dev/null -w "CSS: %{http_code} %{size_download}" http://127.0.0.1/CGCPT/assets/index-CyBTKcDw.css'
)
